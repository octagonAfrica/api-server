...
class LoginController extends Controller
{
    public function login(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json(
                [
                    'success' => false,
                    'message' => "Login failed. Please Input username/Password or Check password length"
                ],
                401
            );
        } else {
    ...
    public function sendLoginVerification(Request $request)
    {
        ...
        // $sql = "SELECT TOP 1 * FROM sys_users_tb where user_email = '$email'";
        // $result = $db_opas->dbSelectRows($sql);
        // $detail = $result[0];
        $user_id = $user->user_id;
