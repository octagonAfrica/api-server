...
class RegisterContoller extends Controller
{
    ...
    public function registerUser(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $add_user_members_tb = DB::connection('mydb_sqlsrv')->insert(
                'INSERT INTO members_tb(m_scheme_code,m_number,m_county, m_name, m_gender, m_email, m_phone, m_id_number) VALUES (?, ?, ?, ?, ?, ?,?,?)',
                ['APP01','MemberNumber',$country, $username, $gender, $email, $phone_number, $ID]
            );
            if ($add_user_members_tb && $add_user) {
                $mailData = [
                    'fullnames' => $fullnames,
                    'username' => $username,
                    'email' => $email,
